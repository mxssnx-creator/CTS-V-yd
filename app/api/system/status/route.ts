...
export async function GET(request: NextRequest) {
  try {
    ...
    const activeConnections = allConnections.filter((c) => isTruthyFlag(c.is_enabled) && isTruthyFlag(c.is_active))
    ...
    const systemStatus = {
      ...
      connections: {
        ...
        enabled: allConnections.filter((c) => isTruthyFlag(c.is_enabled)).length,
        disabled: allConnections.filter((c) => !isTruthyFlag(c.is_enabled)).length,
        byExchange,
        byApiType,
      },
      health: {
